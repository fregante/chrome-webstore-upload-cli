...
async function doAutoPublish({ apiConfig, trustedTesters, deployPercentage, zipPath }) {
...
async function doUpload({ apiConfig, zipPath }) {
    console.log(`Uploading ${path.basename(zipPath)}`);
...
async function doPublish({ apiConfig, trustedTesters, deployPercentage }) {
    console.log('Publishing');
...
export function errorHandler(error) {
    ...
    if (error?.name === 'HTTPError') {
        ...
        const { clientId, refreshToken } = config.apiConfig;
        ...
        if (response.error_description === 'Bad Request') {
            const { clientId } = config.apiConfig;
            console.error(
...
export async function init() {
    ...
    config = await createConfig(command, cli.flags);

    const { isUpload, isPublish, autoPublish } = config;

    if (isUpload && autoPublish) {
        await doAutoPublish(config);
    } else if (isUpload) {
        await doUpload(config);
    } else if (isPublish) {
        await doPublish(config);
    }
}
